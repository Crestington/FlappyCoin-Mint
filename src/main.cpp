...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
	...
	nRewardCoinYear = MAX_FLAP_PROOF_OF_STAKE;

	if(nHeight < YEARLY_BLOCKCOUNT)
		nRewardCoinYear = 4 * MAX_FLAP_PROOF_OF_STAKE;
	else if(nHeight < (2 * YEARLY_BLOCKCOUNT))
		nRewardCoinYear = 3 * MAX_FLAP_PROOF_OF_STAKE;
	else if(nHeight < (3 * YEARLY_BLOCKCOUNT))
		nRewardCoinYear = 2 * MAX_FLAP_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in FlappyCoin
    bool fStrictPayToScriptHash = true; // Always active in FlappyCoin
